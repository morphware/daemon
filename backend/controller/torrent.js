...
import("webtorrent-hybrid").then((mod) => {
  ...
  instance.findOrSeed = function (path) {
    return new Promise((resolve, reject) => {
      this.seed(path, function (torrent) {
        ...
        let infoHash = error.message.replace(
      ...
      for (let link of links) {
        this.add(link, { path }, (torrent) => {
          ...
          torrent.on("download", function (bytes) {
            console.log("torrent is downloading", bytes);
          });
  ...
  instance.removeTorrent = function (torrent) {
    return new Promise((resolve, reject) => {
      ...
      torrent
        ...
        .on("warning", (error) => {
          console.error("WARNING Failed to stop torrenting", error);
          reject(error);
        })
        .on("error", (error) => {
          console.error("ERROR Failed to stop torrenting", error);
          reject(error);
        });
    });
  }.bind(instance);

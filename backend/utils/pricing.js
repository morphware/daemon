...
async function calculateBid(trainingTimeInHours) {
  try {
    ...
    const competingAWSCUDAPricePerHour = await findClosestCUDACorePerHourPrice(
      workerCUDACores
    );
    console.log("competingAWSCUDAPricePerHour: ", competingAWSCUDAPricePerHour);
    const biddingValueUSD =
      competingAWSCUDAPricePerHour *
      workerCUDACores *
      parseInt(trainingTimeInHours) *
      0.8;
    ...
    try {
      ...
      fs.createReadStream("./us_east_1.csv")
        ...
        .on("data", (currInstance) => {
          ...
          if (
            instanceCUDACores < workerCUDACores &&
            instanceCUDACores >= closestMatchCUDACores
          ) {
          ...
          //If found instance with exact same amount of CUDA Cores
            ...
            //Is the # of CPU cores is closer to the workers CPU Core count, save as ref
              ...
              instanceCPUCores >= closestMatchCPUCores &&
              instanceCPUCores <= workerCPUCores
            ) {
              closestMatch = currInstance;
            }
